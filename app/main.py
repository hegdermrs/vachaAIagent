...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler(Path(__file__).resolve().parent.parent / "logs" / "app.log", encoding="utf-8"),
    ],
)
...
async def login_page(request: Request):
    ...
    return HTMLResponse(content=html.replace("{{ next }}", next_url).replace("{{ error }}", ""))
...
async def login(request: Request):
    ...
    if password != DASHBOARD_PASSWORD:
        ...
        with open(template_path, encoding="utf-8") as f:
            html = f.read()
        return HTMLResponse(content=html.replace("{{ next }}", next_url).replace(
